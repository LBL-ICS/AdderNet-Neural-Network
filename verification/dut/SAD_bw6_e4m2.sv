  ...
  always @(posedge clock) begin
    if (reset) begin
      ...
      if (io_valid_in) begin
        automatic logic        aIsInf = (&(io_in_a[5:2])) & ~(|(io_in_a[1:0]));
        automatic logic        bIsInf = (&(io_in_b[5:2])) & ~(|(io_in_b[1:0]));
        automatic logic        oneInf = aIsInf ^ bIsInf;
        automatic logic        _GEN =
          (&(io_in_a[5:2])) & (|(io_in_a[1:0])) | (&(io_in_b[5:2])) & (|(io_in_b[1:0]))
          | aIsInf & bIsInf;
        automatic logic [2:0]  normFracA = {|(io_in_a[5:2]), io_in_a[1:0]};
        automatic logic [2:0]  normFracB = {|(io_in_b[5:2]), io_in_b[1:0]};
        automatic logic [5:0]  eA =
          (|(io_in_a[5:2])) ? {2'h0, io_in_a[5:2]} - 6'h7 : 6'h3A;
        automatic logic [5:0]  eB =
          (|(io_in_b[5:2])) ? {2'h0, io_in_b[5:2]} - 6'h7 : 6'h3A;
        automatic logic        aIsLarger =
          eA == eB ? normFracA > normFracB : $signed(eA) > $signed(eB);
        automatic logic [5:0]  _expDiffS_T = eA - eB;
        automatic logic [5:0]  expDiffU =
          $signed(_expDiffS_T) < 6'sh0 ? 6'h0 - _expDiffS_T : _expDiffS_T;
        automatic logic [2:0]  _GEN_0 = aIsLarger ? normFracB : normFracA;
        automatic logic        overShift = expDiffU > 6'h5;
        automatic logic [5:0]  coreSh = overShift ? 6'h5 : expDiffU;
        automatic logic [5:0]  shifted = {_GEN_0, 3'h0} >> coreSh;
        automatic logic [68:0] _mask_T = 69'h1 << coreSh;
        ...
        automatic logic [5:0]  newExp =
          isZero ? 6'h0 : (aIsLarger ? eA : eB) - {3'h0, shiftAmt};
        automatic logic        _GEN_1 = $signed(newExp) < -6'sh6;
        automatic logic [5:0]  _under_T = 6'h3A - newExp;
        automatic logic [5:0]  denFull =
          shiftedExt >> (_under_T > 6'h6 ? 6'h6 : _under_T);
        ...
        sr_data_0 <=
          _GEN | oneInf
            ? (_GEN ? 6'h3E : oneInf ? 6'h3C : 6'h0)
            : {isZero
                 ? 4'h0
                 : _GEN_1
                     ? {3'h0, _sum_T_1[2]}
                     : newExp[3:0] + {3'h0, _sum_T_3[2]} + 4'h7,
               isZero
                 ? 2'h0
                 : _GEN_1
...
module FPAdder_6_1(
  ...
  wire       _lzc_io_isZero;
  wire [5:0] eA = (|(io_in_a[5:2])) ? {2'h0, io_in_a[5:2]} - 6'h7 : 6'h3A;
  wire [5:0] eB = (|(io_in_b[5:2])) ? {2'h0, io_in_b[5:2]} - 6'h7 : 6'h3A;
  wire [5:0] _expDiffs_T = eA - eB;
  wire       expAGtB = $signed(eA) > $signed(eB);
  wire [5:0] expDiff = $signed(_expDiffs_T) < 6'sh0 ? 6'h0 - _expDiffs_T : _expDiffs_T;
  wire [2:0] _GEN = {|(io_in_a[5:2]), io_in_a[1:0]};
  wire [2:0] _GEN_0 = {|(io_in_b[5:2]), io_in_b[1:0]};
  wire [2:0] _GEN_1 = expAGtB ? _GEN_0 : _GEN;
  wire [3:0] smallPre = {1'h0, _GEN_1};
  wire [2:0] _sVal_T_1 = expDiff > 6'h4 ? 3'h4 : expDiff[2:0];
  ...
  always @(posedge clock) begin
    ...
    if (io_valid_in) begin
      ...
      automatic logic        anyInfIn =
        (&(io_in_a[5:2])) & ~(|(io_in_a[1:0])) | (&(io_in_b[5:2])) & ~(|(io_in_b[1:0]));
      automatic logic [5:0]  expOut = expAGtB ? eA : eB;
      ...
      automatic logic [5:0]  _GEN_3 = {3'h0, fracSum[2:0]};
      automatic logic [5:0]  _GEN_4 = {4'h0, _lzc_io_out};
      automatic logic [5:0]  _shiftedFrac_T_2 = _GEN_3 << _GEN_4;
        ...
        + {3'h0,
           (fracSum[3]
             ...
             & ((fracSum[3]
                ...
                | shiftedFrac[0])};
      automatic logic [5:0]  _expAfter_T_1 =
        (fracSum[3] ? expOut + 6'h1 : _lzc_io_isZero ? 6'h0 : expOut - _GEN_4)
        + {5'h0, _roundedWide_T_1[3]};
      automatic logic        _GEN_5 = shiftedFrac == 3'h0;
      automatic logic        _GEN_6 = $signed(_expAfter_T_1) < -6'sh6;
      automatic logic [5:0]  _sRaw_T = 6'h3A - expOut + {5'h0, fracSum[2]};
      automatic logic        _GEN_7 = $signed(_expAfter_T_1) > 6'sh7;
      automatic logic [5:0]  den = _GEN_3 >> ((|(_sRaw_T[5:2])) ? 6'h3 : _sRaw_T);
      sr_data_0 <=
        anyNaNIn | anyInfIn
          ? (anyNaNIn ? 6'h3E : anyInfIn ? 6'h3C : 6'h0)
          : {_GEN_5 | _GEN_6 ? 4'h0 : _GEN_7 ? 4'hF : _expAfter_T_1[3:0] + 4'h7,
             _GEN_5
               ? 2'h0
               : _GEN_6
                   ? den[1:0]
