  ...
  always @(posedge clock) begin
    if (reset) begin
      ...
      if (io_valid_in) begin
        automatic logic        aIsInf = (&(io_in_a[5:3])) & ~(|(io_in_a[2:0]));
        automatic logic        bIsInf = (&(io_in_b[5:3])) & ~(|(io_in_b[2:0]));
        automatic logic        oneInf = aIsInf ^ bIsInf;
        automatic logic        _GEN =
          (&(io_in_a[5:3])) & (|(io_in_a[2:0])) | (&(io_in_b[5:3])) & (|(io_in_b[2:0]))
          | aIsInf & bIsInf;
        automatic logic [3:0]  normFracA = {|(io_in_a[5:3]), io_in_a[2:0]};
        automatic logic [3:0]  normFracB = {|(io_in_b[5:3]), io_in_b[2:0]};
        automatic logic [4:0]  eA =
          (|(io_in_a[5:3])) ? {2'h0, io_in_a[5:3]} - 5'h3 : 5'h1E;
        automatic logic [4:0]  eB =
          (|(io_in_b[5:3])) ? {2'h0, io_in_b[5:3]} - 5'h3 : 5'h1E;
        ...
        automatic logic        overShift = expDiffU > 5'h6;
        automatic logic [4:0]  coreSh = overShift ? 5'h6 : expDiffU;
        automatic logic [6:0]  shifted = {_GEN_0, 3'h0} >> coreSh;
        automatic logic [37:0] _mask_T = 38'h1 << coreSh;
        ...
        automatic logic [6:0]  denFull =
          shiftedExt >> ((|(_under_T[4:3])) ? 5'h7 : _under_T);
        ...
        sr_data_0 <=
          _GEN | oneInf
            ? (_GEN ? 6'h3C : oneInf ? 6'h38 : 6'h0)
            : {isZero
                 ? 3'h0
                 : _GEN_1
                     ? {2'h0, _sum_T_1[3]}
                     : newExp[2:0] + {2'h0, _sum_T_3[3]} + 3'h3,
               isZero
                 ? 3'h0
                 : _GEN_1
...
module FPAdder_6_1(
  ...
  wire       _lzc_io_isZero;
  wire [4:0] eA = (|(io_in_a[5:3])) ? {2'h0, io_in_a[5:3]} - 5'h3 : 5'h1E;
  wire [4:0] eB = (|(io_in_b[5:3])) ? {2'h0, io_in_b[5:3]} - 5'h3 : 5'h1E;
  ...
  wire [3:0] _GEN = {|(io_in_a[5:3]), io_in_a[2:0]};
  wire [3:0] _GEN_0 = {|(io_in_b[5:3]), io_in_b[2:0]};
  wire [3:0] _GEN_1 = expAGtB ? _GEN_0 : _GEN;
  wire [4:0] smallPre = {1'h0, _GEN_1};
  wire [2:0] _sVal_T_1 = expDiff > 5'h5 ? 3'h5 : expDiff[2:0];
  ...
  always @(posedge clock) begin
    ...
    if (io_valid_in) begin
      ...
      automatic logic [4:0]  _expAfter_T_1 =
        (fracSum[4]
           ? expOut + 5'h1
           : _lzc_io_isZero ? 5'h0 : expOut - {2'h0, _lzc_io_out})
        + {4'h0, _roundedWide_T_1[4]};
      automatic logic        _GEN_3 = shiftedFrac == 4'h0;
      automatic logic        _GEN_4 = $signed(_expAfter_T_1) < -5'sh2;
      automatic logic [4:0]  _sRaw_T = 5'h1E - expOut + {4'h0, fracSum[3]};
      automatic logic        _GEN_5 = $signed(_expAfter_T_1) > 5'sh3;
      automatic logic [4:0]  den =
        {1'h0, fracSum[3:0]} >> (_sRaw_T > 5'h4 ? 5'h4 : _sRaw_T);
      sr_data_0 <=
        anyNaNIn | anyInfIn
          ? (anyNaNIn ? 6'h3C : anyInfIn ? 6'h38 : 6'h0)
